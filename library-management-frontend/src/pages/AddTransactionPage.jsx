...
export default function AddTransactionPage() {
  ...
  const handleAddTransaction = async () => {
    if (transactionType === 'borrow') {
      ...
      try {
        const url = `http://localhost:8080/api/transactions/issue?bookId=${selectedBook}&userId=${selectedUser}`;
      ...
      try {
        const url = `http://localhost:8080/api/transactions/return?transactionId=${transactionId}`;
        const response = await fetch(url, {
          method: 'POST',
        });
